...
import {
  loadFaceMeshModel,
  disposeFaceMeshModel,
  FaceLandmarkerResult,
} from "../models/faceMeshLoader";
...
if (typeof window !== "undefined") {
  ...
  console.error = (...args: unknown[]) => {
    ...
    if (
      ...
      message.includes("TensorFlow Lite XNNPACK")
    ) {
...
export function useFaceMesh(
  ...
  const detectFaces = useCallback(async () => {
    ...
    try {
      ...
      const results: FaceLandmarkerResult = detector.detectForVideo(
        videoElement,
        timestamp,
      );
      ...
      if (results.faceLandmarks && results.faceLandmarks.length > 0) {
        const predictions: FacePrediction[] = results.faceLandmarks.map(
          (landmarks, index) => {
  ...
  useEffect(() => {
    return () => {
      ...
      // Dispose to ensure new config (e.g., numFaces) is applied on next mount.
      void disposeFaceMeshModel();
    };
  }, []);
